...
function PreviewCard({ src, title, desc, delay }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y:-6, borderColor:"rgba(6,182,212,0.24)", background:"rgba(255,255,255,0.035)" }}
      ...
      <div style={{ padding:10, borderBottom:"1px solid rgba(255,255,255,0.06)", background:"rgba(255,255,255,0.025)" }}>
        <div style={{ height:isMobile ? 240 : 360, borderRadius:16, overflow:"hidden", background:"rgba(255,255,255,0.035)", border:"1px solid rgba(255,255,255,0.06)" }}>
          <img
            src={src}
            alt={title}
...
export default function ProjectClinic({ onBack }) {
  const isMobile = useIsMobile();
  ...
  const previewImages = [
    { src:"/images/clinic-schema.png", title:"ERD Diagram", desc:"An Entity-Relationship Diagram showing the clinic entities, attributes, primary keys, and relationships between patients, doctors, appointments, prescriptions, departments, and medical records." },
    ...
    <div style={{ background:"#030303", color:"#f5f5f7", fontFamily:"'SF Pro Display',-apple-system,BlinkMacSystemFont,sans-serif", minHeight:"100vh", overflowX:"hidden", cursor:"default" }}>
      ...
      <motion.header style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, backdropFilter:"blur(32px)", WebkitBackdropFilter:"blur(32px)", borderBottom:"1px solid rgba(255,255,255,0.055)", background:navBg }}>
        <div style={{ maxWidth:1200, margin:"0 auto", padding:isMobile ? "0 18px" : "0 48px", height:62, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
          ...
          <MagneticLink href="https://github.com/JolianHabib/Medical-Clinic-Database" target="_blank"
            style={{ fontSize:isMobile ? 10 : 11, fontWeight:700, letterSpacing:"0.1em", background:"rgba(6,182,212,0.12)", color:COLOR, padding:isMobile ? "9px 13px" : "10px 22px", borderRadius:999, border:"1px solid rgba(6,182,212,0.25)", whiteSpace:"nowrap" }}>
            GitHub ↗
          </MagneticLink>
        </div>
      </motion.header>
      ...
      <motion.section style={{ position:"relative", zIndex:1, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:isMobile ? "110px 20px 70px" : "130px 48px 90px", textAlign:"center" }}>
        <motion.div style={{ y:heroY }}>
          ...
          <motion.h1 initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2, duration:0.8, ease:[0.16,1,0.3,1] }}
            style={{ fontSize:isMobile ? "clamp(38px,15vw,68px)" : "clamp(42px,7vw,100px)", fontWeight:700, lineHeight:0.95, letterSpacing:"-0.055em", color:"#fff", marginBottom:32, maxWidth:1000 }}>
            Medical Clinic
            <span style={{ display:"block", background:`linear-gradient(135deg,${COLOR},rgba(6,182,212,0.3))`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
              Database
            </span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.65, duration:0.7 }}
            ...
            {["PostgreSQL","PLpgSQL","Schema Design","Triggers","Constraints","Normalization","Analytical Queries"].map(t => <Pill key={t} label={t} />)}
          </motion.div>

          <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.8, duration:0.7 }}>
            <MagneticLink href="https://github.com/JolianHabib/Medical-Clinic-Database" target="_blank"
              style={{ gap:9, background:COLOR, color:"#000", fontSize:14, fontWeight:700, padding:isMobile ? "15px 28px" : "16px 36px", borderRadius:999, boxShadow:"0 0 60px rgba(6,182,212,0.25)", width:isMobile ? "100%" : "auto", maxWidth:isMobile ? 280 : "none" }}>
              View on GitHub
        ...
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <motion.div initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.8 }}
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:isMobile ? 12 : 20, flexWrap:"wrap" }}>
              {[["Patients","Demographics & history"],["Doctors","Specializations & schedule"],["Departments","Clinic structure"]].map(([l,s],i)=>(
                <div key={l} style={{ display:"flex", flexDirection:"column", alignItems:"center" }}>
                  <ArchNode label={l} sub={s} color="#06b6d4" delay={0.1+i*0.05} />
                  <Arrow />
                </div>
              ))}
            </div>
            ...
            <div style={{ display:"flex", justifyContent:"center" }}>
              <ArchNode label="Appointments" sub="Core junction table" color="#3b82f6" delay={0.25} />
            </div>
            <div style={{ display:"flex", justifyContent:"center" }}><Arrow /></div>

            {/* Sub-entities */}
            <div style={{ display:"flex", justifyContent:"center", gap:isMobile ? 12 : 20, flexWrap:"wrap" }}>
              <ArchNode label="Prescriptions" sub="Medications" color="#f59e0b" delay={0.3} />
              <ArchNode label="Billing" sub="Payments & invoices" color="#f59e0b" delay={0.35} />
              <ArchNode label="Medical Records" sub="Diagnoses & notes" color="#f59e0b" delay={0.4} />
            </div>
            <div style={{ display:"flex", justifyContent:"center" }}><Arrow /></div>

            {/* DB engine */}
            <div style={{ display:"flex", justifyContent:"center" }}>
              <ArchNode label="PostgreSQL + PLpgSQL" sub="Triggers · Constraints · Indexes" color="#10b981" delay={0.45} />
            </div>
      ...
      <section style={{ position:"relative", zIndex:1, padding:isMobile ? "0 20px 70px" : "0 48px 90px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <motion.div initial={{ opacity:0, y:40 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:1 }}
            style={{ position:"relative", padding:isMobile ? "44px 22px" : "80px", border:"1px solid rgba(6,182,212,0.12)", borderRadius:32, background:"rgba(6,182,212,0.03)", textAlign:"center", overflow:"hidden" }}>
            ...
            <div style={{ position:"relative", zIndex:1 }}>
              ...
              <MagneticLink href="https://github.com/JolianHabib/Medical-Clinic-Database" target="_blank"
                style={{ gap:10, background:COLOR, color:"#000", fontSize:15, fontWeight:700, padding:isMobile ? "16px 28px" : "18px 44px", borderRadius:999, boxShadow:"0 0 80px rgba(6,182,212,0.15)", width:isMobile ? "100%" : "auto", maxWidth:isMobile ? 280 : "none" }}>
                View on GitHub
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 00-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0020 4.77 5.07 5.07 0 0019.91 1S18.73.65 16 2.48a13.38 13.38 0 00-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 005 4.77a5.44 5.44 0 00-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 009 18.13V22"/></svg>
              </MagneticLink>
            </div>
